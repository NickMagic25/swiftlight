...
@MainActor final class ClientModel: ObservableObject {
    ...
    var statisticsSampler = StreamStatisticsSampler()
    ...
    private func remoteApplicationControlFailed(_ error: any Error, generation: UInt64) {
        guard generation == controlGeneration, !Task.isCancelled else { return }
        if HostControlFailurePolicy.requiresTrustReview(error) {
            remoteApplicationAction = nil
            controlFailed(error, generation: generation)
        } else {
            message = error.localizedDescription
    ...
    private func prepareLaunch(_ app: RemoteApp, quitting expectedAppID: Int? = nil) {
        ...
        controlTask = Task {
            ...
            do {
                ...
                remoteApplicationControlFailed(error, generation: generation)
            }
        }
    }

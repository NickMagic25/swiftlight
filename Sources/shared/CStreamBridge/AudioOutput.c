...
#if !TARGET_OS_OSX
    // The app activates AVAudioSession and requests its preferred output count
    // before creating RemoteIO. Read the hardware side's accepted format: a
    // requested surround mix does not imply that this route has 6 or 8 outputs.
    AudioStreamBasicDescription hardware_format = {0};
    UInt32 hardware_size = sizeof(hardware_format);
    unit_status = AudioUnitGetProperty(o->unit, kAudioUnitProperty_StreamFormat,
        kAudioUnitScope_Output, 0, &hardware_format, &hardware_size);
    if (unit_status != noErr) return unit_status;
    uint32_t physical_channels = hardware_format.mChannelsPerFrame;
    if (!physical_channels) return kAudioUnitErr_FormatNotSupported;
    if (physical_channels < o->channels)
        o->output_channels = physical_channels >= 6 ? 6 : physical_channels >= 2 ? 2 : 1;
#endif

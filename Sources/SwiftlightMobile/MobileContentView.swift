...
struct MobileContentView: View {
        ...
        if model.selectedHost == nil {
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 20) {
                    ...
                    Text("During a stream, tap with three fingers to open controls and disconnect. With VoiceOver, use the Stream Controls action.")
                        .font(.footnote)
    ...
    private func start(_ app: RemoteApp) {
        guard let client = model.client, let host = model.hostInfo, launchTask == nil else { return }
        ...
        launchTask = Task {
            ...
            do {
                ...
                guard launchGeneration == generation, !Task.isCancelled, !(error is CancellationError) else { return }
                if error is RunningApplicationConflict {
                    launchMessage = "Another application is running on this computer. Resume that application, or quit it on the computer before starting another."

...
struct MobileDisplayProbe: UIViewRepresentable {
    let onChange: @MainActor (DisplayGeometry, MobileHDRDisplayCapabilities) -> Void
...
@MainActor final class MobileDisplayProbeView: UIView {
    var onChange: (@MainActor (DisplayGeometry, MobileHDRDisplayCapabilities) -> Void)?
    private var pendingUpdate = false
    private var lastGeometry: DisplayGeometry?
    private var lastHDRCapabilities: MobileHDRDisplayCapabilities?
    ...
    func scheduleUpdate() {
        ...
        Task { @MainActor [weak self] in
            ...
            guard geometry != lastGeometry || hdr != lastHDRCapabilities else { return }
            lastGeometry = geometry
            lastHDRCapabilities = hdr
            onChange?(geometry, hdr)
        }
    }
}

...
common-c is a pristine Git submodule pinned to `62e066388f1a1b133e0bee947b9a374311a3354b`, with its exact recursive ENet/nanors pins. The build applies the explicit, targeted patch series to generated sources, covering callback lifetime/atomic arbitration and read-only clock/socket access. See `dependencies.md` and `transport.md`. Opus1.5.2 and OpenSSL3.6.4 are built statically from SHA-verified source archives. The local app's GPL/dependency notices are bundled. Apple targets beyond macOS require future application/dependency slices.
